...
class PatientGUI:
    def mainFrame(self):
        ...
        self.patientWindow.configure(bg="#333333")

        title = tk.Label(self.patientWindow,text="WELCOME!",bg="#333333",fg="#FFFFFF",font=("Franklin Gothic Heavy",25))
        title.grid(column=0,row=0,pady=5,columnspan=2,sticky="news",padx=90)

        #ID and entry
        self.Id = StringVar()
        id = tk.Label(self.patientWindow,text="Your ID",bg="#333333",fg="#FFFFFF",font=("Arial",16,"bold"))
    ...
    def findByID2(self,searchedID):
        ...
        self.patientInfoWindow.configure(bg="#EAE0DA")
        ...
        Welcome = tk.Label(self.patientInfoWindow,text="Your information",bg="#EAE0DA",font=("Arial",20,"bold"))
        Welcome.grid(column=3,row=0,columnspan=5,sticky="news",pady=10,ipadx=50)

        id = tk.Label(self.patientInfoWindow,text="ID",bg="#EAE0DA",font=("Arial",14))
        id.grid(column=0,row=1,padx=40,sticky=tk.W)
        IDValue = tk.Label(self.patientInfoWindow,text=searchedID,bg="#EAE0DA",fg="#913175",font=("Arial",14))
        IDValue.grid(column=1,row=1,sticky=tk.W)

        age = tk.Label(self.patientInfoWindow,text="Age",bg="#EAE0DA",font=("Arial",14))
        age.grid(column=3,row=1,padx=40,sticky=tk.E)
        ageValue = tk.Label(self.patientInfoWindow,text=searchedAge,bg="#EAE0DA",fg="#913175",font=("Arial",14))
        ageValue.grid(column=4,row=1,sticky=tk.W)

        name = tk.Label(self.patientInfoWindow,text="Name",bg="#EAE0DA",font=("Arial",14))
        name.grid(column=0,row=2,sticky=tk.W,padx=40,columnspan=2)
        nameValue = tk.Label(self.patientInfoWindow,text=searchedName,bg="#EAE0DA",fg="#913175",font=("Arial",14))
        nameValue.grid(column=1,row=2,sticky=tk.W,columnspan=4)

        address = tk.Label(self.patientInfoWindow,text="Address",bg="#EAE0DA",font=("Arial",14))
        address.grid(column=0,row=3,columnspan=2,sticky=tk.W,padx=40)
        addressValue = tk.Label(self.patientInfoWindow,text=searchedAddress,bg="#EAE0DA",fg="#913175",font=("Arial",14))
        addressValue.grid(column=1,row=3,sticky=tk.W,columnspan=4)

        condition = tk.Label(self.patientInfoWindow,text="Condition",bg="#EAE0DA",font=("Arial",14))
        condition.grid(column=0,row=4,columnspan=2,sticky=tk.W,padx=40)
        conditionValue = tk.Label(self.patientInfoWindow,text=searchedCondition,bg="#EAE0DA",fg="#913175",font=("Arial",14))
        conditionValue.grid(column=1,row=4,columnspan=4,sticky=tk.W)

        creator = tk.Label(self.patientInfoWindow,text="Creator",bg="#EAE0DA",font=("Arial",14))
        creator.grid(column=0,row=5,sticky=tk.W,padx=40)
        creatorName = tk.Label(self.patientInfoWindow,text=searchedCreator,bg="#EAE0DA",fg="#913175",font=("Arial",14,"bold"))
        creatorName.grid(column=1,row=5,sticky=tk.W)

        #Advice text
        advice = tk.Label(self.patientInfoWindow,text="Advice",bg="#EAE0DA",font=("Arial",14))

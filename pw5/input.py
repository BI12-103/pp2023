...
def inputSt(Sts,stdscr):
    ...
    while True:
        ...
        id,name,dob = newStudent(len(stList)+1,stdscr)
        ...
        try:
            stList.append(temp)
            mng.updateStudent(temp) #Write to txt
            del(temp)
...
def inputCourse(courseList,stdscr):
    ...
    while True:
        ...
        if len(cL) == newNumCourse:
            break
        #Input and check if ID is dub
        id,name = newCourse(len(cL)+1,stdscr)
            ...
            for x in range(len(stList)):
                while True:
                    try:
                        ...
                        marked.append(temp)
                        mng.updateMarks(courseM,stList[x].getID(),stList[x].getName(),m)
                        del(temp)
                        break
                    except ValueError:
                        pass
